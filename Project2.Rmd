...
---
## Introduction
This analysis uses the ToothGrowth dataset in the R datasets package to determine the statistical significance of two different delivery methods of Vitamin C and their effect on tooth growth in guinea pigs.

## Summary of Data
The data used in this analysis shows the response in length of the teeth in 10 guinea pigs to three dose levels of Vitamin C delivered in two different ways: orange juice and ascorbic acid.  

There are 60 observations of three variables in the dataset: length of the tooth (number), supplement type (factor with two levels: VC or OJ), dose of Vitamin C (number in milligrams).  
...
tidyTooth$`delivery method`[tidyTooth$"delivery method" == "OJ"] <- "Orange Juice"
tidyTooth$`delivery method`[tidyTooth$"delivery method" == "VC"] <- "Ascorbic Acid"
tidyTooth$`delivery method` <- as.factor(tidyTooth$`delivery method`)
```

## Exploratory Analysis
First, we will take a look at a plot of the data to see if we can spot any trends worth investigating in the data.
```{r echo = TRUE, warning = FALSE}
library(ggplot2)
ggplot(data = tidyTooth) + geom_point(aes(x = `dosage size`, y = length, color = `delivery method`))
```  

From the data, we can see what appears to be a trend of increasing length of teeth when we increase the dosage of Vitamin C. From just a cursory review, it appears that the Ascorbic Acid delivery method has a greater effect on teeth length, but we'll take a look at the data a bit more closely to make that call.
...
```{r echo = TRUE}
t.test(tidyTooth$length[tidyTooth$"delivery method" == "Ascorbic Acid"], tidyTooth$length[tidyTooth$"delivery method" == "Ascorbic Acid"], alternative = "two.sided", paired= FALSE)
